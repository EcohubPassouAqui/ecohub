...
local function BuildSection(sectionname, PageScroll)
	...
	function SecObj:addDropdown(name, list, callback)
		...
		DIL.Parent = DS2
		DIL.Padding = UDim.new(0, 0)
		DIL.SortOrder = Enum.SortOrder.LayoutOrder

		local tH = math.min(#list * 24, 130)
		...
		for i, item in ipairs(list) do
			...
			local function UpdateOpt()
				if isSelected then
					Tw(OptLbl, {TextColor3 = C.TEXT}, TI.Fast)
					Tw(Selector, {BackgroundTransparency = 0}, TI.Fast)
				else
					Tw(OptLbl, {TextColor3 = C.DIM}, TI.Fast)
					Tw(Selector, {BackgroundTransparency = 1}, TI.Fast)
				end
			end
			...
			Opt.MouseEnter:Connect(function()
				if not isSelected then
					Tw(OptLbl, {TextColor3 = C.TEXT}, TI.Fast)
					Tw(Opt, {BackgroundColor3 = C.HOVER}, TI.Fast)
				end
			end)
			Opt.MouseLeave:Connect(function()
				Tw(Opt, {BackgroundColor3 = C.SECT}, TI.Fast)
				UpdateOpt()
			end)
			Opt.MouseButton1Click:Connect(function()
				for _, o in pairs(optionObjects) do
					o.selected = false
					o.update()
				end
				isSelected = true
