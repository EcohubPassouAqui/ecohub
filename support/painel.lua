...
local theme = getgenv().theme or {
    ...
    Accent = Color3.fromRGB(130, 130, 180),
    NotSelectedTab = Color3.fromRGB(70, 70, 100),
    TextColor = Color3.fromRGB(255,255,255),
    SubTextColor = Color3.fromRGB(200,200,200),
    ButtonTop = Color3.fromRGB(50, 50, 64),
    ButtonBottom = Color3.fromRGB(58, 58, 85),
}
...
do
    function library.init(title, version, id, position, size)
        ...
        local TopBarVersion = util.new("TextLabel", {
        ...
        table.insert(connections, input.InputBegan:Connect(function(inp, gpe)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 and not gpe then
                ...
                local bottomRight = topLeft + Vector2.new(MasterContainer.AbsoluteSize.X, 30)
    ...
    function library:AddTab(title, desc)
        ...
        local UIListLayout = self.TabSelectContainer:FindFirstChildOfClass("UIListLayout")
        if UIListLayout then
            self.TabSelectContainer.CanvasSize = UDim2.new(0, 0, 0, UIListLayout.AbsoluteContentSize.Y + 10)
        end
...
do
    function tab.new(library, title, desc, id)
        local TabSelector, TabSelectorColour = util.new("TextButton", {
...
do
    function panel.new(tab, panelProperties, id)
        ...
        local UIListLayout = PanelContainer:FindFirstChildOfClass("UIListLayout")
        if UIListLayout then
            UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                PanelContainer.CanvasSize = UDim2.new(0, 0, 0, UIListLayout.AbsoluteContentSize.Y + 14)
            end)
        end
    ...
    function panel.AddToggle(panel, data)
        ...
        if data.colorpicker then
            ...
            local ColorpickerWindowContainer, ColorpickerWindow = util.new("TextButton", {
            ...
            local currentHue, currentSat, currentVal = 0,0,0
            local function hueFromPercent(percent, override)
                currentHue = percent
                SatValWindow.ImageColor3 = Color3.fromHSV(currentHue, 1, 1)
            ...
            panel.tab.library.registerExternalClickFunction(function(inp)
                self.expandedColor = false
                ColorpickerWindowContainer.Visible = self.expandedColor
            end)
    ...
    function panel.AddSlider(panel, data)
        ...
        local function renderFromPercent(percent, override)
            ...
            SliderboxFill:TweenSize(UDim2.new(percent, -2, 1, -2), "In", "Linear", 0.05, true, function()
                if SliderboxFill.AbsoluteSize.X < 0 then SliderboxFill.Size = UDim2.new(0,0,1,-2) end
            end)
    ...
    function panel.AddDropdown(panel, data)
        ...
        local DropdownItem = util.new("TextButton", {
            LayoutOrder = 1,
            Size = UDim2.new(1, 0, 0, 20),
            ZIndex = 61,
            BackgroundColor3 = theme.InteractableOutline
        ...
        local function setSelected(count)
        ...
        return {
            ...
            getSelected = function() return self.selected, self.options[self.selected] end,
    ...
    function panel.AddSelection(panel, data)
        ...
        local function render()
            ...
            for count, text in pairs(self.options) do
                ...
                DropdownItem.MouseButton1Click:Connect(function()
                    ...
                    if not selected and found ~= nil then
                        table.remove(self.selected, found)
                    elseif selected and found == nil then
        ...
        local function setSelected(table)
            self.selected = table
            render()
        end
        ...
        return {
            ...
            setSelected = setSelected,
            getSelected = function() return self.selected end,
            setDropdownExpanded = function(expanded)
                self.expanded = expanded
                renderDropdown()
            end,
