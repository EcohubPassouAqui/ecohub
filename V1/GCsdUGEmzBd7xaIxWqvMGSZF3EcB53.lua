...
local function _Err(scope, msg, ...)
	...
	table.insert(_ErrorLog, { scope = scope, message = full, time = os.clock() })
	print(full)
end
...
if not _iconsOk then
	_Err("Icons", "failed to load icons: %s", tostring(_iconsData))
end
...
local T = {
	...
	AccentSub  = Color3.fromRGB(140, 140, 140),
	...
	tabIdle    = Color3.fromRGB(120, 120, 120),
	tabHov     = Color3.fromRGB(200, 200, 200),
...
local function MkToggle(par, text, default, cb)
	...
	btn.MouseButton1Click:Connect(function()
		state = not state
		refresh(state)
		local ok, err = pcall(function() if cb then cb(state) end end)
		if not ok then _Err("Toggle", "callback failed: %s", err) end
	end)
	HoverEl(btn, f)
...
local function MkCheckbox(par, text, default, cb)
	...
	btn.MouseButton1Click:Connect(function()
		...
		refresh(state)
		local ok, err = pcall(function() if cb then cb(state) end end)
		if not ok then _Err("Checkbox", "callback failed: %s", err) end
	end)
	HoverEl(btn, f)
...
local function MkButton(par, text, cb)
	...
	btn.MouseButton1Click:Connect(function()
		local ok, err = pcall(function() if cb then cb() end end)
		if not ok then _Err("Button", "callback failed: %s", err) end
	end)
...
local function MkSlider(par, text, minV, maxV, defV, cb)
	...
	local function setVal(v)
		...
		local ok, err = pcall(function() if cb then cb(val) end end)
		if not ok then _Err("Slider", "callback failed: %s", err) end
	end
	...
	local function buildItems()
		...
		for i, opt in ipairs(options) do
			...
			local ob = N("TextButton", {
				Size=UDim2.fromScale(1,1), BackgroundTransparency=1, Text="", ZIndex=14, Parent=row,
			})
			ob.MouseEnter:Connect(function()
				if not isSel then
					Tw(row, {BackgroundTransparency=0, BackgroundColor3=Color3.fromRGB(65,65,65)}, 0.07)
				end
			end)
	...
	UIS.InputBegan:Connect(function(inp)
		...
		if inp.UserInputType == Enum.UserInputType.Keyboard then
			...
			if inp.KeyCode == Enum.KeyCode.Escape then
				...
				local ok, err = pcall(function() if cb then cb(key) end end)
				if not ok then _Err("Keybind", "callback failed: %s", err) end
			end
...
local function MkColorPicker(par, text, defCol, cb)
	...
	Grad(pickWrap, Color3.fromRGB(55,55,55), Color3.fromRGB(35,35,35), 90)

	local pickInner = N("Frame", {
		Size=UDim2.new(1,0,0,PICK_H), BackgroundTransparency=1,
	...
	local function makeRGBRow(labelText, yOff, fillCol, initVal)
		...
		N("TextLabel", {
			...
			TextColor3=T.subText, TextSize=10, Font=Enum.Font.GothamBold,
			TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=pickInner,
		})
		local badge2 = N("Frame", {
		...
		Corner(badge2, 4)
		Stroke(badge2, T.elBorder, 1, 0.15)
		local valLbl2 = N("TextLabel", {
			Size=UDim2.fromScale(1,1), BackgroundTransparency=1,
			Text=tostring(initVal), TextColor3=T.Accent,
			TextSize=9, Font=Enum.Font.GothamBold,
			TextXAlignment=Enum.TextXAlignment.Center, ZIndex=14, Parent=badge2,
		})
	...
	if hexInput then
		hexInput.FocusLost:Connect(function(enter)
			...
			if ok2 and typeof(c2) == "Color3" then
				rV = math.round(c2.R*255) gV = math.round(c2.G*255) bV = math.round(c2.B*255)
	...
	return {
		Set = function(_, c)
			color=c rV=math.round(c.R*255) gV=math.round(c.G*255) bV=math.round(c.B*255)
			applySlider(sR,rV) applySlider(sG,gV) applySlider(sB,bV)
			syncAll(false)
		end,
...
local function MkParagraph(par, title, content)
	...
	_order = _order + 1
	local f = N("Frame", {
		Size=UDim2.new(1,0,0,dynH), BackgroundColor3=T.elBg,
		BorderSizePixel=0, ClipsDescendants=false,
		LayoutOrder=_order, ZIndex=3, Parent=par,
	})
	Corner(f, 6)
	Stroke(f, T.elBorder, 1, 0.28)
	Grad(f, Color3.fromRGB(28,28,28), Color3.fromRGB(17,17,17), 90)
	...
	local titleLbl = N("TextLabel", {
		Size=UDim2.new(1,-16,0,14), Position=UDim2.new(0,11,0,8),
		BackgroundTransparency=1, Text=title or "",
		TextColor3=T.Accent, TextSize=10, Font=Enum.Font.GothamBold,
		TextXAlignment=Enum.TextXAlignment.Left, TextTruncate=Enum.TextTruncate.AtEnd,
		ZIndex=4, Parent=f,
	})
	local contentLbl = N("TextLabel", {
		Size=UDim2.new(1,-22,1,-26), Position=UDim2.new(0,11,0,24),
...
local function MkSecObj(scroll)
	...
	function obj:AddToggle(text, default, callback)
		return MkToggle(scroll, text, default, callback)
	end
	function obj:AddCheckbox(text, default, callback)
		return MkCheckbox(scroll, text, default, callback)
	end
	...
	function obj:AddDropdown(text, opts, defV, cb)
		return MkDropdown(scroll, text, opts, defV, cb)
	end
	function obj:AddKeybind(text, defKey, callback)
		return MkKeybind(scroll, text, defKey, callback)
	end
	function obj:AddColorPicker(text, defCol, cb)
		return MkColorPicker(scroll, text, defCol, cb)
	end
	function obj:AddParagraph(title, content)
		return MkParagraph(scroll, title, content)
	end
	function obj:AddSection(name)
		MkSection(scroll, name or "Section")
		return MkSecObj(scroll)
	end
	return obj
end
...
function lib:CreateWindow(opts)
	...
	function window:SetTitle(newTitle)
		if titleLbl then titleLbl.Text = newTitle or "" end
	end

	function window:SetSubtitle(newSubtitle)
		if subtitleLbl then subtitleLbl.Text = newSubtitle or "" end
	end

	function window:GetErrors()
		return _ErrorLog
	end
	...
	function window:AddTab(opts2)
		...
		function tabObj:SetTitle(newTitle)
			if lbl then lbl.Text = newTitle or "" end
			if tip then tip.Text = newTitle or "" end
			tab.name = newTitle or ""
		end
	...
	function window:Destroy()
		local ok, err = pcall(function() gui:Destroy() end)
		if not ok then _Err("Destroy", "failed to destroy gui: %s", err) end
	end
